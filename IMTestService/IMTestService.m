...
- (id)initWithServiceApplication:(id<IMServiceApplication>)serviceApplication
{
    ...
    if ((self=[super init])) {
        ...
        IOBluetoothDevice *btdev= [IOBluetoothDevice deviceWithAddressString:@"00:00:00:00:00:00"];
        IOBluetoothSDPUUID *sdpuuid = [IOBluetoothSDPUUID uuid16:kBluetoothSDPUUID16ServiceClassMessageAccessServer];
        IOBluetoothSDPServiceRecord *record = [btdev getServiceRecordForUUID:sdpuuid];
        _obexHandler = [OBEXSessionHandler initWithSDPServiceRecord:record];
        [_obexHandler connectToBTDevice];
        [_obexHandler setMasDelegate:self];
        [_obexHandler setPath:@"telecom"];
        [_obexHandler setPath:@"msg"];
        _mObexMNSServer = [[OBEXMNSServer alloc] initWithMNSDelegate:self];
        [_mObexMNSServer publishService];
        [_obexHandler sendNotificationRegistration];
...
- (void)dataLoaded:(NSString *)messageData {
    NSLog(@"things happened2???");
    NSAttributedString *attrString = [[NSAttributedString alloc] initWithString:messageData];
    [self.application plugInDidReceiveMessage:[IMServicePlugInMessage servicePlugInMessageWithContent:attrString] fromHandle:@"log"];
}

- (void)receivedMessage:(NSString *)messageHandle {
    NSAttributedString *attrString = [[NSAttributedString alloc] initWithString:messageHandle];
    NSLog(@"plugin received msg: %@", attrString);
    NSLog(@"requesting msg: %@", attrString);
    [_obexHandler getMessage:messageHandle withDelegate:self];
    [self.application plugInDidReceiveMessage:[IMServicePlugInMessage servicePlugInMessageWithContent:attrString] fromHandle:@"log"];
}
...
- (oneway void)requestGroupList
{
    ...
    NSArray *capabilities = @[IMHandleCapabilityFileTransfer, IMHandleCapabilityChatRoom];
    NSDictionary *changes = @{
                              IMHandlePropertyAvailability: @(IMHandleAvailabilityAvailable),
                              IMHandlePropertyCapabilities: capabilities
                              };
    for (NSString* user in [self usersForGroup:nil])
    {
        NSLog(@"updating properties for %@", user);
        //[self log:[NSString stringWithFormat:@"updating properties for %@", user]];
        [self.application plugInDidUpdateProperties:changes ofHandle:user];
    }
...
{
    NSLog(@"sendMessage: %@ to: %@", message.content, handle);
    [self.application plugInDidSendMessage:message toHandle:handle error:nil];
    NSLog(@"sendMessage: echoing...");
    if([handle hasPrefix:@"+"]) {
        [_obexHandler sendMessage:message.content.string phonenumber:[handle substringFromIndex:1]];
    }
    [self.application plugInDidReceiveMessage:message fromHandle:handle];
}
...
- (oneway void)startOutgoingFileTransferSession:(IMServicePlugInOutgoingFileTransferSession *)session toHandle:(NSString *)handle
{
    ...
    [self log:@"outgoing transfer starting?"];
    [self.application plugInDidAcceptOutgoingFileTransferSession:session];
}
...
- (oneway void)declineChatRoomInvitation:(NSString *)roomName {
    [self log:@"declineChatRoomInvitation"];
...
- (oneway void)inviteHandles:(NSArray *)handles toChatRoom:(NSString *)roomName withMessage:(IMServicePlugInMessage *)message {
    [self log:@"inviteHandles"];
    NSLog(@"inviteHandles: %@ toChatroom: %@, withMessage: %@", handles, roomName, message);
    [self.application handles:handles didJoinChatRoom:roomName];

}
- (oneway void)joinChatRoom:(NSString *)roomName {
    [self log:@"joinChatRoom"];
    [self.application plugInDidJoinChatRoom:roomName];
    NSLog(@"joinChatRoom: %@", roomName);

}
- (oneway void)leaveChatRoom:(NSString *)roomName {
    [self log:@"leaveChatRoom"];
    NSLog(@"leaveChatRoom: %@", roomName);

}
- (oneway void)sendMessage:(IMServicePlugInMessage *)message toChatRoom:(NSString *)roomName {
    [self log:@"sendMessage"];
    NSLog(@"sendMessage: %@ toChatRoom: %@", message, roomName);
    [self.application plugInDidSendMessage:message toChatRoom:roomName error:nil];
    [self.application plugInDidReceiveMessage:message forChatRoom:roomName fromHandle:@"log"];
